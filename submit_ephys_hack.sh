...
for probe in a b; do
  for shank_num in 0 1 2 3; do
    ...
    bsub -J "$JOB_NAME" \
         ...
         bash -lc "conda activate spikenv411 && python -u '$SCRIPT_DIR/run_pipeline.py' '$DAY_DIR' '$probe' '$shank_num'"
  done
done

# do we need to point to some sort of path to python itself for the conda env? or just conda activate?
#bash -lc "source /path/to/conda/etc/profile.d/conda.sh && conda activate spikenv411 && python -u '$SCRIPT_DIR/run_pipeline.py' '$DAY_DIR' '$probe' '$shank_num'"
