...
for probe in a b; do
    for shank in 1 2 3 4; do
        ...
        PROC_JOB_ID=$(bsub -w "done($MERGE_JOB_ID)" -n 32 -o "$LOG_DIR/preprocess_${probe}_shank_${shank}_${CHUNK}.log" \
                    "python ${CODEPATH}/preprocess.py $FILEPATH $probe $shank $DATAPATH $CHUNK" | awk '{print $2}' | tr -d '<>')
        ...
        DREDGE_JOB_ID=$(bsub -w "done($PROC_JOB_ID)" -n 32 -o "$LOG_DIR/motion_${probe}_shank_${shank}_${CHUNK}.log" \
                        "python ${CODEPATH}/motion.py $FILEPATH $probe $shank $DATAPATH $CHUNK" | awk '{print $2}' | tr -d '<>')
        ...
        for dredge in true false; do
            echo "Running detection for probe $probe shank $shank chunk $CHUNK dredge $dredge" >> "$LOG_DIR/pipeline.log" 2>&1

            # Submit detection job dependent on motion job completion
            KS_JOB_ID=$(bsub -w "done($DREDGE_JOB_ID)" -n 12 -gpu "num=1" -q "gpu_tesla" \
                        -o "$LOG_DIR/detection_${probe}_shank_${shank}_chunk_${CHUNK}_dredge_${dredge}_${CHUNK}.log" \
                        "python ${CODEPATH}/detection.py $FILEPATH $dredge $probe $shank $DATAPATH $CHUNK" | awk '{print $2}' | tr -d '<>')

            if [ -z "$KS_JOB_ID" ]; then
                echo "Failed to submit detection job for probe $probe shank $shank chunk $CHUNK dredge $dredge" >> "$LOG_DIR/pipeline.log" 2>&1
                exit 1
            fi
        done
    done
