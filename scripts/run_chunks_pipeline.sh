        ...
        for chunk_idx in $(seq 0 $(($N_CHUNKS - 1))); do
            for dredge in true false; do
                ...
                KS_JOB_ID=$(bsub -w "done($DREDGE_JOB_ID)" -n 12 -gpu "num=1" -q "gpu_tesla" \
                            -o "$LOG_DIR/detection_${probe}_shank_${shank}_chunk_${chunk_idx}_dredge_${dredge}.log" \
                            "python ${CODEPATH}/detection.py $FILEPATH $dredge $probe $shank $chunk_idx $DATAPATH $CHUNK" | awk '{print $2}' | tr -d '<>')
