...
def main() -> None:
    ...
    run([PYTHON, "-s", "-u", str(SCRIPT_DIR / "pipeline" / "run_shank.py"), args.data_dir, args.probe, args.shank])
    run([PYTHON, "-s", str(SCRIPT_DIR / "pipeline" / "postproc.py"), args.data_dir, args.probe, args.shank])
